#!/usr/bin/env python3
"""Refined sweep: extend AC to 10 GHz to measure exact GBW and PM."""
...
os.chdir(ROOT)
WORK_DIR = ROOT / "output" / "opamp_two_stage_refine"
...
def build_testbench(cc, m1, m5, m3, m6, m7, stop="10G"):
...
def main():
    print("=" * 70)
    print("  Refined Sweep: AC to 10 GHz")
    print("=" * 70)
    ...
    configs = [
        ("0.5p", 16, 32, 16,  6,  8, "0.5p m16", "5G"),
        ("0.3p", 32, 64, 32,  6,  8, "0.3p m32", "5G"),
        ("0.3p", 32, 64, 32, 10, 14, "0.3p m32 rebal", "5G"),
        ("0.2p", 48, 96, 48,  8, 12, "0.2p m48", "5G"),
        ("0.2p", 48, 96, 48, 12, 16, "0.2p m48 rebal", "5G"),
        ("0.15p", 48, 96, 48, 12, 16, "0.15p m48", "10G"),
        ("0.1p",  64, 128, 64, 12, 16, "0.1p m64", "10G"),
        ("0.1p",  64, 128, 64,  8, 12, "0.1p m64 rebal", "10G"),
    ]

    results_table = []
    for cc, m1, m5, m3, m6, m7, desc, stop in configs:
        print(f"\n[Cc={cc} m1={m1} m6={m6}/{m7} stop={stop}]")
        tb = build_testbench(cc, m1, m5, m3, m6, m7, stop=stop)
        tb_path = WORK_DIR / f"tb_{desc.replace(' ', '_')}.scs"
        ...
        if metrics:
            ...
            if metrics['gbw'] and metrics['gbw'] > 1e9:
                gbw_str = f"{metrics['gbw']/1e9:.2f} GHz"
            pm_str = f"{metrics['pm']:.1f} deg" if metrics['pm'] else "N/A"
            g1g_str = f"{metrics['gain_1g']:.1f} dB" if metrics['gain_1g'] else "N/A"
            print(f"  Gain: {metrics['dc_gain']:.1f} dB  "
                  f"G@1G: {g1g_str}  "
                  f"GBW: {gbw_str}  "
                  f"PM: {pm_str}  "
                  f"VOUT: {metrics['vout_dc']:.3f} V")
            results_table.append((cc, m1, m6, m7, metrics))

    print("\n" + "=" * 70)
    print("  Refined Summary")
    print("=" * 70)
    print(f"  {'Cc':<8s} {'m1':>4s} {'m6':>4s} {'m7':>4s}  "
          f"{'Gain':>7s}  {'G@1G':>7s}  {'GBW':>10s}  {'PM':>7s}  {'VOUT':>6s}")
    print("  " + "-" * 65)
    for cc, m1, m6, m7, m in results_table:
        gbw_s = "N/A"
        if m['gbw']:
            gbw_s = f"{m['gbw']/1e9:.2f} GHz" if m['gbw'] > 1e9 else f"{m['gbw']/1e6:.0f} MHz"
        pm_s = f"{m['pm']:.1f}" if m['pm'] else "N/A"
        g1g_s = f"{m['gain_1g']:.1f}" if m['gain_1g'] else "N/A"
        print(f"  {cc:<8s} {m1:>4d} {m6:>4d} {m7:>4d}  "
              f"{m['dc_gain']:6.1f} dB  {g1g_s:>5s} dB  {gbw_s:>10s}  {pm_s:>5s} deg  {m['vout_dc']:5.3f} V")
    print("=" * 70)
