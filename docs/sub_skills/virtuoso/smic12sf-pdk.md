---
name: smic12sf-pdk
description: "SMIC 12nm FinFET PDK reference: layer definitions, device parameters, schematic-symbol terminal names, and Python API helpers."
---
...
| Cell | Description | Terminals | Typical Use |
|------|-------------|-----------|-------------|
| `npn08` | 0.8V NPN | C, B, E | Core-voltage bandgap |
| `npn18` | 1.8V NPN | C, B, E | I/O-voltage bandgap |
| `pnp08` | 0.8V PNP (vertical, C=substrate) | C, B, E | Core-voltage bandgap |
| `pnp18` | 1.8V PNP (vertical, C=substrate) | C, B, E | I/O-voltage bandgap |

> **Note:** Cells with suffix `_ckt` (e.g. `pnp18a8_ckt`) appear in the library cell list but **lack schematic/symbol views** — they cannot be placed in schematics. For schematic design, always use `pnp08`, `pnp18`, `npn08`, `npn18`.

For vertical PNPs in CMOS, the collector is the p-substrate — always connect C to VSS (GND).

---

## Schematic Symbol Terminal Names

When connecting PDK devices in schematics, terminals must match exactly (case-sensitive):

| Device | Terminals | Notes |
|--------|-----------|-------|
| **MOSFET** (n18_ckt, p18_ckt, etc.) | G, D, S, B | B = bulk/body. NMOS B→VSS, PMOS B→VDD |
...
> **Symbol viewType:** All smic12sf PDK symbols use `viewType="schematicSymbol"` (not "schematic"). Use `dbOpenCellViewByType(lib, cell, "symbol", "schematicSymbol", "r")` to open them programmatically.
