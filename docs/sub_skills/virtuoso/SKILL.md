...
| `references/smic12sf-pdk.md` | **SMIC 12nm FinFET PDK** — layer definitions, device generators, parameter reference |
...
- **`15_smic12sf_pdk.py`** — **SMIC 12nm FinFET PDK** — create MOS devices, resistors, capacitors with typed layer constants
...
**SMIC 12nm FinFET Process Design Kit** optimized for `virtuoso-bridge`. Provides typed layer constants, device generators, and parameter helpers.

### Layer Constants

Layer names are fully typed — no more string typos or layer number guessing:

```python
from virtuoso_bridge.virtuoso.layout import SMIC12SF

# Basic layers
layer, purpose = SMIC12SF.AA, SMIC12SF.DRAWING    # ("AA", "drawing")
layer, purpose = SMIC12SF.GT, SMIC12SF.DRAWING    # ("GT", "drawing")
layer, purpose = SMIC12SF.NW, SMIC12SF.DRAWING    # ("NW", "drawing")

# Metal layers (1-7)
M1, M2, M3 = SMIC12SF.M1, SMIC12SF.M2, SMIC12SF.M3
M4, M5, M6, M7 = SMIC12SF.M4, SMIC12SF.M5, SMIC12SF.M6, SMIC12SF.M7

# Top thick metals
TM1, TM2 = SMIC12SF.TM1, SMIC12SF.TM2

# Vias (0-6 + TV + BV)
V0, V1, V2, V3 = SMIC12SF.V0, SMIC12SF.V1, SMIC12SF.V2, SMIC12SF.V3
V4, V5, V6 = SMIC12SF.V4, SMIC12SF.V5, SMIC12SF.V6
TV1, TV2 = SMIC12SF.TV1, SMIC12SF.TV2

# Purposes
DRAWING, PIN, LABEL = SMIC12SF.DRAWING, SMIC12SF.PIN, SMIC12SF.LABEL

# lpp() helper for common patterns
lpp = SMIC12SF.lpp(SMIC12SF.M1, SMIC12SF.PIN)  # ("M1", "pin")
```

Full layer list: AA, FIN, GT, DG, P2, SN, SP, NW, DNW, NPAA, PPAA, M0, M0C, M1-M7, TM1, TM2, ALPA, PA, BUMP, V0-V6, TV1, TV2, BV1, BV2.
...
with client.layout.edit(LIB, CELL) as lay:
    # Standard threshold (SVT) — most common
    lay.add(smic12sf_nmos_svt("MN0", 0, 0, l=0.014, w=0.1, nf=1))
    lay.add(smic12sf_pmos_svt("MP0", 0, 2, l=0.014, w=0.1, nf=1))

    # Low threshold (LVT) — higher leakage, higher speed
    lay.add(smic12sf_nmos_lvt("MN1", 5, 0, l=0.014, w=0.1, nf=4))
    lay.add(smic12sf_pmos_lvt("MP1", 5, 2, l=0.014, w=0.1, nf=4))

    # Full control: voltage, dnw, multipler
    lay.add(smic12sf_create_nmos(
        "MN2", 10, 0,
        l=0.014, w=0.1, nf=10, m=2,  # 2 parallel instances × 10 fingers
        vth="ulvt",                    # ulvt | lvt | svt | hvt
        voltage="08",                  # "08" = 0.8V core, "18" = 1.8V IO
        dnw=False,                     # deep N-well isolation
    ))

    # PMOS has same API (no dnw option)
    lay.add(smic12sf_create_pmos(
        "MP2", 10, 2,
        l=0.014, w=0.2, nf=20, m=1,
        vth="hvt",                     # high threshold, low leakage
        voltage="08",
    ))
```

**Voltage domains:**
- `"08"` — 0.8V core devices (default)
- `"18"` — 1.8V I/O devices

**Threshold options (ordered by leakage/speed):**
- `"ulvt"` — Ultra Low Vt, fastest, highest leakage
- `"lvt"` — Low Vt, fast, high leakage
- `"svt"` — Standard Vt, balanced (default)
- `"hvt"` — High Vt, slow, lowest leakage

**MOS parameters:**
- `l` — gate length (um), default 0.014
- `w` — gate width per finger (um), default 0.1
- `nf` — number of fingers, default 1
- `m` — multiplier (parallel instances), default 1
- `dnw` — deep N-well (NMOS only), default False
...
# Metal resistors (rm1-rm7, rtm1, rtm2, ralpa)
lay.add(smic12sf_create_resistor("R1", 0, 0, res_type="rm1", w=0.1, l=10, m=1))
lay.add(smic12sf_create_resistor("R2", 5, 0, res_type="rtm1", w=0.5, l=50))

# High-res poly
lay.add(smic12sf_create_resistor("R3", 10, 0, res_type="rhrpo", w=0.5, l=100))

# N-well resistor
lay.add(smic12sf_create_resistor("R4", 15, 0, res_type="rnwsti", w=1, l=5))
```
...
# Standard 2-terminal MOM cap
lay.add(smic12sf_create_mom_cap("C0", 0, 0, ports=2, w=1, l=1, nf=10))

# High-quality (HQ) version
lay.add(smic12sf_create_mom_cap("C1", 5, 0, ports=2, high_quality=True, w=2, l=2))

# Ultra-low capacitance
lay.add(smic12sf_create_mom_cap("C2", 10, 0, ports=2, ultra_low=True, w=0.2, l=0.2))

# Multi-terminal (2, 3, 4, 5 ports)
lay.add(smic12sf_create_mom_cap("C3", 15, 0, ports=5, w=1, l=1))
```
...
# Available: ndio08, ndio18, pdio08, pdio18, dnwdio, nwdio, rwdio
lay.add(smic12sf_create_diode("D0", 0, 0, diode_type="ndio08", w=0.5, l=0.5))
lay.add(smic12sf_create_diode("D1", 5, 0, diode_type="pdio18", w=1, l=1))
```

### Quick Reference

| Device Category | Prefix | Common Types |
|-----------------|--------|--------------|
| NMOS | `n*_ckt` | n08, nlvt08, nhvt08, nulvt08, n18 |
| PMOS | `p*_ckt` | p08, plvt08, phvt08, pulvt08, p18 |
| Resistor | `r*_ckt` | rm1-rm7, rtm1, rtm2, rhrpo, rnwsti |
| Capacitor | `mom*` | mom_2t, mom_3t, mom_4t, mom_5t |
| Diode | `*dio*` | ndio08, ndio18, pdio08, pdio18 |
| BJT | `npn*/pnp*` | npn08, npn18, pnp08, pnp18 |
| ESD | `*_esd` | n18_esd, ndio18_esd, npn5_esd |

**PDK library name:** `smic12sf`

**Default view:** `layout`

**Manufacturing grid:** 1nm (0.001 um)

**Tech nodes:** FinFET 12nm/14nm nominal
